...
one (Data nums syms) = sum $ fmap snd $ (nums Map.!) <$> keys
  where
    adjs = concatMap adjacent8 $ Map.keys syms
    keys = head <$> uniquify (fst <$> mapMaybe (nums Map.!?) adjs)


two (Data nums syms) = sum $ fmap product $ mapMaybe sat adjs
  where
    adjs = map adjacent8 $ Map.keys $ Map.filter (== '*') syms
    sat adjs = if length x == 2 then Just (fmap snd x) else Nothing
      where x = uniquify $ mapMaybe (nums Map.!?) adjs
